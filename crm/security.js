(()=>{
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();const code=input.value.trim();if(!/^\d{4,8}$/.test(code)){err.textContent='Le code doit contenir entre 4 et 8 chiffres.';return}const h=await hash(code);if(!localStorage.getItem('crmAccessHash'))localStorage.setItem('crmAccessHash',h);else if(h!==localStorage.getItem('crmAccessHash')){err.textContent='Code incorrect.';input.value='';input.focus();return}sessionStorage.setItem('crmUnlocked','1');lock.remove();document.body.style.overflow='';askMicConsent();});
  if(sessionStorage.getItem('crmUnlocked')==='1'){lock.remove();document.body.style.overflow='';setTimeout(askMicConsent,150)}
  function askMicConsent(){const old=document.querySelector('#micConsent');if(old)old.remove();sessionStorage.removeItem('crmMicAllowed');const dg=document.createElement('dialog');dg.id='micConsent';dg.innerHTML=`<div class="mic-card"><div class="mic-icon">🎙️</div><h2>Autorisation du microphone</h2><p>Le microphone sert uniquement à dicter votre journée dans le planning. Aucun enregistrement audio n’est conservé par le CRM.</p><div class="mic-actions"><button class="mic-allow" type="button">Autoriser le microphone</button><button class="mic-deny" type="button">Continuer sans microphone</button></div><div class="mic-status"></div></div>`;document.body.appendChild(dg);dg.showModal();dg.querySelector('.mic-allow').onclick=()=>{sessionStorage.setItem('crmMicAllowed','1');dg.close();dg.remove()};dg.querySelector('.mic-deny').onclick=()=>{sessionStorage.setItem('crmMicAllowed','0');dg.close();dg.remove()}}
  document.addEventListener('click',e=>{if(e.target.closest('#micConsent,#crmLock'))return;const mic=e.target.closest('[id*="mic" i], [class*="mic" i], [aria-label*="micro" i], [title*="micro" i]');if(mic&&sessionStorage.getItem('crmMicAllowed')!=='1'){e.preventDefault();e.stopImmediatePropagation();askMicConsent()}},true);
})();