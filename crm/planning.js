(()=>{
  ...
  function render(){
    ...
    for(let hour=7;hour<20;hour++){
      ...
      for(const d of days){const date=localIso(d),es=events.filter(e=>e.date===date&&Math.floor(mins(e.start)/60)===hour);h+=`<div class="day-cell">${es.map(e=>`<div class="plan-event${e.completed?' completed':''}" data-id="${esc(e.id)}" style="height:${Math.max(36,(mins(e.end)-mins(e.start))*.8)}px"><b>${e.completed?'✓ Fait — ':''}${esc(e.start)} ${esc(e.title)}</b>${esc(e.company||'')}</div>`).join('')}</div>`}
    }
  ...
  planningForm.addEventListener('submit',e=>{e.preventDefault();if(e.submitter?.id!=='savePlanning')return;const id=planningId.value||Date.now().toString(),old=events.find(x=>String(x.id)===String(id)),rec={id,companyId:planningCompanyId.value,company:planningCompanySearch.value.trim(),title:planningTitle.value||'Rendez-vous',date:planningDate.value,start:planningStart.value,end:planningEnd.value,dept:planningDepartment.value.trim(),address:planningAddress.value,notes:planningNotes.value,completed:old?.completed||false};events=events.filter(x=>String(x.id)!==String(id));events.push(rec);saveP();monday=getMonday(parseLocal(rec.date));dg.close();render()});
