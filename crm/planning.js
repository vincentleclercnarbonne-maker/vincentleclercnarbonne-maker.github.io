(()=>{
  ...
  function findConflict(candidate,list=events,ignoreId=candidate?.id){
    if(!candidate||candidate.flexibleTime||!candidate.date||!candidate.start||!candidate.end)return null;
    const start=mins(candidate.start),end=mins(candidate.end);
    if(end<=start)return null;
    return list.find(e=>String(e.id)!==String(ignoreId||'')&&e.date===candidate.date&&!e.flexibleTime&&e.start&&e.end&&start<mins(e.end)&&end>mins(e.start))||null;
  }
  window.CRM_PLANNING_FIND_CONFLICT=(candidate,list,ignoreId)=>findConflict(candidate,list||JSON.parse(localStorage.getItem(planningKey)||'[]'),ignoreId);

  function eventCard(e,floating=false,nextStart=''){
    const label=e.flexibleTime?'Sans horaire fixe':e.start;
    const top=floating?0:(mins(e.start)%60)*.8;
    const naturalHeight=Math.max(22,(mins(e.end)-mins(e.start))*.8);
    const availableHeight=nextStart?Math.max(10,(mins(nextStart)-mins(e.start))*.8-2):naturalHeight;
    const height=floating?36:Math.min(naturalHeight,availableHeight);
    const compact=!floating&&height<22;
    const style=floating?'':` style="top:${top}px;height:${height}px"`;
    const fullLabel=`${label} · ${e.company||e.title||'Rendez-vous'}`;
    const content=compact
      ?`<b>${e.completed?'✓ ':''}${esc(fullLabel)}</b>`
      :`<b>${e.completed?'✓ Fait — ':''}${e.tourProspect?`${esc(label)} · Prospect`:`${esc(label)} ${esc(e.title)}`}</b>${e.company?`<span class="${e.tourProspect?'prospect-name':''}">${esc(e.company)}</span>`:''}`;
    return `<div class="plan-event${e.flexibleTime?' flexible':''}${e.tourProspect?' tour-prospect':''}${floating?' floating-event':''}${compact?' compact':''}${e.completed?' completed':''}" data-id="${esc(e.id)}" title="${esc(fullLabel)}"${style}>${content}</div>`;
  }
  ...
  function render(){
    ...
    for(let hour=7;hour<20;hour++){
      ...
      for(const d of days){
        const date=localIso(d),dayEvents=events.filter(e=>e.date===date&&!e.flexibleTime&&e.start&&e.end).sort((a,b)=>mins(a.start)-mins(b.start));
        const es=dayEvents.filter(e=>Math.floor(mins(e.start)/60)===hour);
        h+=`<div class="day-cell">${es.map(e=>{const next=dayEvents.find(x=>mins(x.start)>mins(e.start));return eventCard(e,false,next?.start||'')}).join('')}</div>`;
      }
    }
  ...
  planningForm.addEventListener('submit',e=>{e.preventDefault();if(e.submitter?.id!=='savePlanning')return;events=JSON.parse(localStorage.getItem(planningKey)||'[]');const id=planningId.value||Date.now().toString(),old=events.find(x=>String(x.id)===String(id)),flexibleTime=planningNoFixedTime.getAttribute('aria-pressed')==='true',rec={id,companyId:planningCompanyId.value,company:planningCompanySearch.value.trim(),title:planningTitle.value||'Rendez-vous',date:planningDate.value,start:flexibleTime?'':planningStart.value,end:flexibleTime?'':planningEnd.value,flexibleTime,dept:planningDepartment.value.trim(),address:planningAddress.value,notes:planningNotes.value,completed:old?.completed||false};if(!flexibleTime&&mins(rec.end)<=mins(rec.start))return alert('L’heure de fin doit être après l’heure de début.');const conflict=findConflict(rec,events,id);if(conflict)return alert(`Créneau indisponible : ce rendez-vous chevauche ${conflict.start}–${conflict.end}${conflict.company?` chez ${conflict.company}`:''}. Choisissez un autre horaire.`);events=events.filter(x=>String(x.id)!==String(id));events.push(rec);saveP();monday=getMonday(parseLocal(rec.date));dg.close();render()});
