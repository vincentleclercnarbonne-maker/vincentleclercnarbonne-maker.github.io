(()=>{
  ...
  function eventCard(e,floating=false){
    ...
    const height=e.tourProspect?Math.max(32,(mins(e.end)-mins(e.start))*.8):Math.max(36,(mins(e.end)-mins(e.start))*.8);
    const style=floating?'':` style="top:${top}px;height:${height}px"`;
    return `<div class="plan-event${e.flexibleTime?' flexible':''}${e.tourProspect?' tour-prospect':''}${floating?' floating-event':''}${e.completed?' completed':''}" data-id="${esc(e.id)}"${style}><b>${e.completed?'✓ Fait — ':''}${e.tourProspect?`${esc(label)} · Prospect`:`${esc(label)} ${esc(e.title)}`}</b>${e.company?`<span class="${e.tourProspect?'prospect-name':''}">${esc(e.company)}</span>`:''}</div>`;
  }
